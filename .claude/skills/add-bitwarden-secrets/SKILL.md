...
> **Migrated token contract:** configure the MCP subprocess with `BWS_ACCESS_TOKEN_FILE` pointing at a read-only mounted token file. Direct `BWS_ACCESS_TOKEN` remains a compatibility fallback only; never put a raw token in chat or committed configuration.
