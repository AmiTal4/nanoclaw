...
export async function processQuery(
  ...
  /**
   * Fork feature: flood/auth containment shared with the caller's poll loop,
   * so repeated provider errors inside a turn are gated by the same state
   * that pauses turns between them. Defaults to a fresh gate for tests.
   */
