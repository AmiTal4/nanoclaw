 ...
 *
 * Rejects if `srcPath` cannot be copied — callers decide whether that should
 * surface to the user (the MCP tool validates existence first; the poll-loop
 * consumer logs and moves on so one bad image can't fail the turn).
 */
