...
export class ClaudeProvider implements AgentProvider {
  ...
  query(input: QueryInput): AgentQuery {
    ...
    // The resolver's allowlist already carries one pattern per MCP server;
    // the group's disabled-tool list subtracts from it.
    const allowedTools = this.mcp.allowedTools.filter((tool) => !this.disabledTools.has(tool));
