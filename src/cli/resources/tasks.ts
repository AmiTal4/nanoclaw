...
async function cancelTaskCommand(args: Record<string, unknown>, ctx: CallerContext) {
  if (!bool(args.all)) {
    return mutateTask(args, ctx, (mailbox, id) => mailbox.cancelTask(id), 'cancelled');
  }
...
registerResource({
  ...
  customOperations: {
    ...
    pause: {
      ...
      handler: async (args, ctx) => mutateTask(args, ctx, (mailbox, id) => mailbox.pauseTask(id), 'paused'),
    },
    resume: {
      access: 'open',
      ...
      handler: async (args, ctx) => mutateTask(args, ctx, (mailbox, id) => mailbox.resumeTask(id), 'resumed'),
    },
    delete: {
      access: 'open',
